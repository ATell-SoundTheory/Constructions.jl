...
function render_interactive_assets()
    ...
    try
        ...
        @eval begin
            ...
            Mab = Makie.@lift Makie.Point2f(($A[1]+$B[1])/2, ($A[2]+$B[2])/2)
            Mbc = Makie.@lift Makie.Point2f(($B[1]+$C[1])/2, ($B[2]+$C[2])/2)
            Mca = Makie.@lift Makie.Point2f(($C[1]+$A[1])/2, ($C[2]+$A[2])/2)
            G   = Makie.@lift Makie.Point2f(($A[1]+$B[1]+$C[1])/3, ($A[2]+$B[2]+$C[2])/3)

            fig = Makie.Figure(resolution=(800, 560))
            ax = Makie.Axis(fig[1, 1], aspect=Makie.DataAspect(),
                            xgridvisible=false, ygridvisible=false,
                            limits=((-2,2),(-2,2)))
            Makie.lines!(ax, Makie.@lift [($A[1],$A[2]), ($B[1],$B[2])]; color=:steelblue, linewidth=2)
            Makie.lines!(ax, Makie.@lift [($B[1],$B[2]), ($C[1],$C[2])]; color=:steelblue, linewidth=2)
            Makie.lines!(ax, Makie.@lift [($C[1],$C[2]), ($A[1],$A[2])]; color=:steelblue, linewidth=2)
